...
### Arrays.sort() return 주의사항, 0으로 나누기 처리
* #### Arrays.sort()를 수정할 때, return되는 값은 int타입이어야 한다. 즉, double형끼리의 연산에 대한 양수 판별은 불가하다. 예를 들어 따로 o1[1]과 o2[1]의 차가 양수 일 경우 오름차순을 원한다면 자리변경을 위해 1을, 자리를 바꾸지 않을 거라면 -1을 리턴한=해주어야 한다. 
* #### ex) N=3 stages[1,1,1,1,1] -> fail의 1열은 [5,0,0]이 되고 실패율을 구하는 과정에서 5/5, 0/0, 0/0의 연산이 진행된다. 0으로 나눌 경우 에러가 발생하므로 이에 대한 처리를 따로 해주어야한다. 이 부분 처리를 안해서 몇 개의 테스트 케이스에서 오류가 발생했었다. 
